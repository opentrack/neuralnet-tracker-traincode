...
def timing_and_output(net, x):
    with torch.no_grad():
        ...
        if isinstance(out, dict):
            print(
                f"Output is a dict of tensors of dimensions {[tuple(t.shape) for t in out.values()]}"
            )
        else:
...
def onnx_export_and_inference_speed(net, device: Literal["cpu", "cuda"]):
    ...
    N = 100
    time = timeit.timeit(
        "ort_session.run(None, { 'x' :  x })",
        number=N,
        globals={"ort_session": ort_session, "x": x},
    )
...
def test_pose_network_sanity(tmp_path):
    ...
    net = trackertraincode.neuralnets.models.NetworkWithPointHead(
        config="resnet18", enable_uncertainty=True
    )
    net.eval()
    print("---------- Pose Net ----------")
