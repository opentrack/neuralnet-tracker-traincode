...
from trackertraincode.datasets.randomized import (
    make_concat_dataset_item_sampler,
...
def test_interleaved_datasets2():
    ...
    M = 10
    BS = 10
    ds = ConcatDataset([_TestSet(N1), _TestSet(N2, start=N1)])
    dssamplers = [SequentialSampler(ds.datasets[0]), SequentialSampler(ds.datasets[1])]
    sampler = make_concat_dataset_item_sampler(
        ds, wrapped=dssamplers, weights=[0.5, 0.5], stop_after=M * BS
    )
    y = load_dataset(ds, sampler=sampler, batchsize=BS).numpy()
    NY = M * BS
    assert len(y) == NY
