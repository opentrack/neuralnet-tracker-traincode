...
assert isinstance(
    dshdf5.DatasetEncoding.image_filename, str
), f"Not a string but a {type(dshdf5.DatasetEncoding.image_filename)}"


Data1 = namedtuple(
    "TestData1", ["datanormal", "dataimagepath", "datavarsizeimage1", "datavarsizeimage2"]
)
...
def make_testdata1_hdf5(td: Data1, tmpdir: Path):
    ...
    with h5py.File(tmpdir / "test.h5", "w") as f:
        ...
        varsizeimageds3 = dshdf5.ImageVariableLengthBufferDs.create(
            f, "varsizeimage3", 2, lossy=False
        )
        varsizeimageds4 = dshdf5.ImageVariableLengthBufferDs.create(
            f, "varsizeimage4", 2, lossy=False
        )
...
def test_H5DatasetWrappers(tmpdir: Path):
    ...
    with h5py.File(h5filename, "r") as f:
        ...
        for i, img in enumerate(td.datavarsizeimage1):
            ...
            assert np.allclose(varsizeimageds1[i], img, rtol=1.0, atol=6.0)
            assert np.allclose(varsizeimageds2[i], img, rtol=1.0, atol=6.0)
...
def test_openalldatasets(tmpdir: Path):
    ...
    with h5py.File(h5filename, "r") as f:
        names_datasets = dict(
            dshdf5.open_all_datasets(f, whitelist=list(map(lambda s: "/" + s, f.keys())))
        )
...
def test_ImageDsColorTransform(tmpdir: Path):
    ...
    with h5py.File(tmpdir / "test.h5", "r") as f:
        ...
        ds.monochrome = False
        assert np.allclose(ds[0], np.repeat(img_grey[:, :, None], 3, -1), rtol=1.0, atol=6.0)
        assert np.allclose(ds[1], img_color, rtol=1.0, atol=10.0)
        ds.monochrome = True
        assert np.allclose(ds[0], img_grey, rtol=1.0, atol=6.0)
        assert np.allclose(ds[1], np.average(img_color, axis=-1), rtol=1.0, atol=6.0)
...
@patch(
    "trackertraincode.datasets.dshdf5.TorchHdf5DatasetBase._set_monochrome_flag_to_ds_wrappers",
    side_effect=trackertraincode.datasets.dshdf5.TorchHdf5DatasetBase._set_monochrome_flag_to_ds_wrappers,
    autospec=True,
)
def test_TorchHdf5DatasetBase_monochrome_propagation(
    mock__set_monochrome_flag_to_ds_wrappers, tmpdir: Path
):
    filename, data, sequence_starts = make_test2_hdf5(tmpdir)
    ...
    ds = trackertraincode.datasets.dshdf5.TorchHdf5DatasetBase(
        filename, whitelist=["/test"], monochrome=False
    )
    assert ds[0] is not None
