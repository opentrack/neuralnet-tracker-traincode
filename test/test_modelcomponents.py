...
def sklearn_gmm(rng: np.random.RandomState):
    data = rng.normal(loc=[1.0, 2.0], scale=[3.0, 4.0], size=(100, 2))
...
def test_local_to_global_transform_offset():
    ...
    tr.p.data[:] = torch.tensor([1.0, 3.0, 4.0, 2.0])

    r = Rotation.from_rotvec([0.0, 0.5, 0.0])
    q = r.as_quat().astype(np.float32)
    c = np.asarray([1.0, 2.0, 3.0], dtype=np.float32)

    pred_quat, pred_coord = tr(
        QuatRepr(torch.from_numpy(q)[None, ...]), torch.from_numpy(c)[None, ...], set_id=None
    )
    pred_r = Rotation.from_quat(pred_quat.value.detach().numpy())
    pred_c = pred_coord.detach().numpy()
    expect_scale = 3.0 * np.exp(2.0)
    expect_c = -(r.apply(expect_scale * np.array([3.0, 4.0, 0.0]))[[2, 1]]) + np.array([1.0, 2.0])
