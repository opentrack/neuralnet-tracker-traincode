...
def test_loss():
    ...
    loss = t6dr.rotation_distance_loss(
        torch.from_numpy(rots1.as_matrix()),
        torch.from_numpy(rots2.as_matrix()))

    np.testing.assert_allclose(loss.numpy(), 0.5*(1.-np.cos(delta)), atol=1.e-6)
...
def test_conversions():
    ...
    np.testing.assert_allclose(m.numpy(), ref, atol=1.e-6)

    z = torch.randn((10,6))
    m = t6dr.tomatrix(z)
    np.testing.assert_allclose(torch.linalg.det(m), 1., atol=1.e-6)
    np.testing.assert_allclose(m @ m.mT, torch.eye(3).expand_as(m), atol=2.e-5)

    z = torch.as_tensor([1.,0.,0.,1.,0.,0.])
    m = t6dr.tomatrix(z)
    np.testing.assert_allclose(m, torch.eye(3))

    z = torch.as_tensor([0.,0.,0.,1.,0.,0.])
    m = t6dr.tomatrix(z)
    np.testing.assert_allclose(m, torch.eye(3))

    z = torch.as_tensor([0.,0.,0.,0.,0.,0.])
    m = t6dr.tomatrix(z)
    np.testing.assert_allclose(m, torch.eye(3))
...
def test_orthonormality_loss():
    ...
    np.testing.assert_allclose(loss, torch.zeros((2,)), atol=1.e-6)

    val = torch.as_tensor([1.,0.,0.,1.,0.,0.])
    loss2 = t6dr.orthonormality_loss(val)
    assert loss2.item() > 1.e-2

    val = torch.as_tensor([10.,0.,0.,10.,0.,0.])
    loss3 = t6dr.orthonormality_loss(val)
    assert loss3.item() > loss2.item() + 1.e-2