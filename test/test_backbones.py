...
def test_backbone_output_api(net):
    ...
    assert y.shape == (
        1,
        net.num_features,
    ), f"Bad output shape {y.shape} vs expected {1,net.num_features}"
    if zs is not None:
        expected_sizes = (65, 33, 17, 9, 5)
        for expected_feature_dim, expected_size, z in zip(
            net.num_intermediate_features, expected_sizes, zs
        ):
