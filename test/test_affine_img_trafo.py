...
up_down_sample_configs = [ 
    ('up', { 'upfilter' : 'linear' }, 0.,),
    ('up', { 'upfilter' : 'cubic' }, 0.5,),
    ('up', { 'upfilter' : 'lanczos' }, 0.5,),
    ('down', {'downfilter' : 'gaussian' }, 0.),
    ('down', {'downfilter' : 'hamming' }, 0.),
    ('down', {'downfilter' : 'area' }, 0.)
]
...
def test_image_affine_transform(scaling_way : str):
    ...
    tr = Affine2d.trs(
        translations=torch.tensor([3.,-td.R*0.45]),
        angles = torch.tensor(20.*np.pi/180.),
        scales = torch.tensor(1.5)) @ tr
    ...
    assert torch.sqrt(torch.nn.functional.mse_loss(cv2result, torchresult)).item() <= (2. if scaling_way=='up' else 10.)
...
def test_image_crop_rescale(scaling_way : str):
    ...
    torchresult = croprescale_image_torch(td.batch['image'], roi=td.batch['roi'], new_size=(td.R, td.R))
    points = affinevecmul(tr.tensor(), td.batch['pt3d_68'][...,:2])    
    assert torch.sqrt(torch.nn.functional.mse_loss(cv2result, torchresult)).item() <= 7.
    vis(td, cv2result, points)
    check(td, cv2result, points)
    check(td, torchresult, points)