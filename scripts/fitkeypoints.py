...
class HeadModel(nn.Module):
    ...
    def __init__(self, x, y, s, batchsize):
        ...
        self.out = neuralnets.modelcomponents.HeadPoseLayerQuaternion()
...
def find_poses(pt3d_68):
    ...
    lossvals = eval().detach().cpu().numpy()

    return coord, quat, shapeparams, pts, lossvals
...
def fitall(args):
    with h5py.File(args.filename, 'r+') as f:
        N = f['pt3d_68'].shape[0]
        cs = min(N, 1024)
        ds_quats = f.require_dataset('quats', (N,4), chunks=(cs,4), maxshape=(N,4), dtype='f4')
        ds_coords = f.require_dataset('coords', (N,3), chunks=(cs,3), maxshape=(N,3), dtype='f4')
        ds_shapeparams = f.require_dataset('shapeparams', (N,50), chunks=(cs,50), maxshape=(N,50), dtype='f4')
        ds_fitloss = f.require_dataset('fitloss', (N,), chunks=(cs,), maxshape=(N,), dtype='f4')
        batchsize = 1024*10
        with progressbar.ProgressBar() as bar:
            for i in bar(range(0, N, batchsize)):
                pt3d_68 = np.array(f['pt3d_68'][i:i+batchsize])
                coord, quat, shapeparams, fitpt3d, fitloss = find_poses(pt3d_68)
                ds_quats[i:i+batchsize] = quat
                ds_coords[i:i+batchsize] = coord
                ds_shapeparams[i:i+batchsize] = shapeparams
                ds_fitloss[i:i+batchsize] = fitloss
