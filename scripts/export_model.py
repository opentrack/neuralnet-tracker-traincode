...
def clear_denormals(state_dict, threshold=1.0e-20):
...
def quantize_backbone(original: trackertraincode.neuralnets.models.NetworkWithPointHead):
    ...
    config = QConfig(
        activation=fake_quantize.FusedMovingAvgObsFakeQuantize.with_args(
        ...
        weight=fake_quantize.FakeQuantize.with_args(
            observer=fake_quantize.MovingAveragePerChannelMinMaxObserver,
            quant_min=-128,
            quant_max=127,
            dtype=torch.qint8,
    ...
    convnet = prepare_fx(
        fuse_convbn(torch.fx.symbolic_trace(original.convnet)), qconfig_mapping, example_input
    )
...
def compare_network_outputs(torchmodel, ort_session, inputs):
    ...
    outputs = ort_session.run(
        None, {k: v.detach().numpy() for k, v in zip(torchmodel.input_names, inputs)}
    )
    # Outputs better be the same ...
    for a, b, name in zip(torch_out, outputs, torchmodel.output_names):
        a = a.detach().numpy()
        if not np.allclose(a, b, 1.0e-4):
            delta = np.amax(np.abs(a - b))
            print(f"WARNING Torch output of {name}, differs from Onnx output by max {delta}")


@torch.no_grad()
def convert_posemodel_onnx(
    net: nn.Module, filename, for_opentrack=True, quantize=False, fp16=False
):
    ...
    inputs = (torch.randn(B, 1, net.input_resolution, net.input_resolution),)
    ...
    dynamic_axes = (
        None if for_opentrack else {k: {0: "batch"} for k in (net.input_names + net.output_names)}
    )
    ...
    if ort is not None:
        ort_session = ort.InferenceSession(
            onnxmodel.SerializeToString(), providers=["CPUExecutionProvider"]
        )
...
def convert_localizer(args):
    ...
    if ort is not None:
        ...
        outputs = ort_session.run(None, {"x": x.detach().numpy()})
...
if __name__ == "__main__":
    ...
    if args.posemodelfilename:
        ...
        convert_posemodel_onnx(
            net,
            args.posemodelfilename,
            for_opentrack=not args.full,
