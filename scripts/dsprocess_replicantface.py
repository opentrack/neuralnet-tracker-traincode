...
if __name__ == '__main__':
    ...
    is_valid_mask = np.asarray([ check_valid(npz_to_other_files(fn)[0]) for fn in tqdm.tqdm(label_files, desc="validity checking") ])

    seg_rois = np.asarray([generate_roi_from_seg(npz_to_other_files(fn)[1]) for fn in tqdm.tqdm(label_files, desc="mask analysis")])

    quats, xys, pts_rois, landmarks = map(np.asarray,zip(*[convert(lbl) for lbl in tqdm.tqdm(label_files, desc="label conversion")]))
