...
def convert(f):
    def cvtline(line):
        ...
        landmarks = vals[:98*2]
        x_min_rect, y_min_rect, x_max_rect, y_max_rect, pose, expression, illumination, makeup, occlusion, blur, image_name = vals[98*2:]
...
def cropped(img : Union[np.ndarray,Image.Image], roi : np.ndarray, desired_roi_size = 129, padding_factor=0.5, abs_padding=10) -> Tuple[np.ndarray, Affine2d]:
...
def generate_hdf5_dataset(sourcedir, outdir, count=None, min_box_width = 129):
    assert os.path.exists(join(sourcedir,'WFLW_annotations','list_98pt_rect_attr_train_test','list_98pt_rect_attr_train.txt'))
    assert os.path.exists(join(sourcedir,'WFLW_annotations','list_98pt_rect_attr_train_test','list_98pt_rect_attr_test.txt'))
    assert os.path.exists(join(sourcedir,'WFLW_images','0--Parade','0_Parade_marchingband_1_116.jpg'))

    for split in [ 'test', 'train' ]:
        annotationfile = join(sourcedir,'WFLW_annotations','list_98pt_rect_attr_train_test',f'list_98pt_rect_attr_{split}.txt')
        ...
        with h5py.File(join(outdir, f'wflw_{split}.h5'), 'w') as f:
            ...
            for i, path, roi in tqdm.tqdm(zip(itertools.count(), paths, rois), total=N):
                ...
                imgs, trafo = cropped(imgs, roi, desired_roi_size=224, padding_factor=0.5, abs_padding=10)
                ds_img[i] = imgs
...
if __name__ == '__main__':
    ...
    parser.add_argument('destination', help='destination directory', type=str, nargs='?', default=None)
