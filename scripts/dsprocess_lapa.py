...
def discover_items(source_dir):
    ...
    return DatasetInfo(root, Path(source_dir) / "train" / "landmarks", items)
...
def improve_roi_with_face_detector(
    img, roi, mtcnn: MTCNN, iou_threshold: float = 0.25, confidence_threshold: float = 0.0
):
...
def cvt_landmarks_68pt(lmk, improved_chin=False):
    ...
    if not improved_chin:
        ...
        chin = lmk[..., :33]
        xs = np.linspace(0.0, 32.0, 33)
        chin = interp1d(xs, chin, kind="quadratic", axis=-1, fill_value="extrapolate")(
            np.linspace(1.5, 32.0 - 1.5, 17)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--only-megaface",
        help="Only generate dataset from the megaface part of the original",
        default=False,
        action="store_true",
    )
