...
def assert_extrinsics_have_identity_rotation(extrinsics: Sequence[Tuple[Any, Affine]]):
    for id_, (rot, _) in extrinsics:
        assert np.allclose(
            rot.as_matrix(), np.eye(3), atol=0.04, rtol=0.0
        ), f"Rotation {rot.as_matrix()} of {id_} is far from identity"


class PinholeCam(object):
    def __init__(self, fov, w, h):
        self.fov = fov
        self.f = 1.0 / math.tan(fov * np.pi / 180.0 * 0.5)
        self.w = w
        self.h = h
        self.aspect = w / h
...
def read_data(
    ...
    cam_extrinsics_inv,
    mtcnn: MTCNN | None,
    box_annotation: tuple[int, int, int, int] | None,
):
    ...
    if mtcnn is not None:
        roi, ok = improve_roi_with_face_detector(
            img, roi, mtcnn, iou_threshold=0.01, confidence_threshold=0.9
        )
        if not ok:
            print(
                f"WARNING: MTCNN didn't find a face that overlaps with the projected head position. Frame {imagefile}."
            )
    else:
        ok = True
...
def generate_hdf5_dataset(
    source_file, outfilename, opal_annotation: str | None, count: int | None = None
):
    mtcnn = None
                ...
                for ident, frames in sequence_frames.items():
                    for fn in frames:
                        sample, ok = read_data(
                            zf,
                            fn,
                            calibration_data[ident],
                            mtcnn,
                            box_annotations[fn] if box_annotations else None,
                        )
                        if ok:
                    ...
                    assert (
                        i != sequence_starts[-1]
                    ), "Every sequence should have at least one good frame!"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--opal-annotation",
        help="Use annotations from opal paper",
        type=str,
        nargs="?",
        default=None,
    )
