...
def _direction_vector_to_pose_matrix(v):
    ...
    pose[:3, :3] = Rotation.from_rotvec(
        _rotvec_between(np.asarray([0.0, 0.0, -1.0]), v)
    ).as_matrix()
    return pose
...
class FaceRender(object):
    def __init__(self):
        ...
        self._mat = pyrender.MetallicRoughnessMaterial(
            doubleSided=True, roughnessFactor=0.1, metallicFactor=0.0
        )
        ...
        self._light = pyrender.light.DirectionalLight(intensity=15.0)
        self._scene.add_node(
            pyrender.Node(
                light=self._light, matrix=_direction_vector_to_pose_matrix([1.0, 0.0, -10.0])
            )
        )
        self._camera_node = pyrender.Node(
            camera=pyrender.PerspectiveCamera(yfov=0.1, znear=1.0, zfar=10.0), matrix=np.eye(4)
        )
        _adjust_camera(self._camera_node, (640, 640, None), 0.0, scale=240)
    ...
    def set(self, xy, scale, rot, shapeparams, image_shape):
        ...
        h, w = image_shape
        _adjust_camera(self._camera_node, (h, w, None), 0.0, scale=w)
        vertices = self._bfm.scaled_vertices + np.sum(
            self._bfm.scaled_bases * shapeparams[:, None, None], axis=0
        )
        normals = _estimate_vertex_normals(vertices, self._bfm.tri)
        self._node.mesh = pyrender.Mesh(
            primitives=[
                pyrender.Primitive(
                    positions=vertices, indices=self._bfm.tri, material=self._mat, normals=normals
                )
            ]
        )
        matrix = np.eye(4)
