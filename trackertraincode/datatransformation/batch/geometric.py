...
def RandomFocusRoi(
    ...
    insert_backtransform=False,
):
    return GeneralFocusRoi(
        MakeRoiRandomizationParameters(
            rotation_aug_angle=rotation_aug_angle, extension_factor=extension_factor
        ),
        new_size,
        roi_variable,
...
def FocusRoi(new_size, extent_factor, roi_variable="roi", insert_backtransform=False):
    return GeneralFocusRoi(
        NoRoiRandomization(extent_factor), new_size, roi_variable, insert_backtransform
    )
...
class MakeRoiRandomizationParameters(object):
    ...
    def __call__(self, B: tuple) -> RoiFocusRandomizationParameters:
        ...
        translations = torch.randn(size=B + (2,)).mul(0.5).clip(-1.0, 1.0)
        angles = (
            self._pick_angles(B, self.rotation_aug_angle)
            if self.rotation_aug_angle
            else torch.zeros(size=B)
        )
    ...
    def _pick_angles(self, B: tuple, angle: float):
        angles = torch.full(B, fill_value=np.pi * angle / 180.0)
        angles *= torch.from_numpy(np.random.choice([-1.0, 1.0], size=B, replace=False))
        angles *= np.random.choice([0.0, 1.0], size=B, replace=False, p=[2.0 / 3, 1.0 / 3])
        return angles
...
class GeneralFocusRoi(object):
    ...
    @staticmethod
    def _compute_view_roi(
        face_bbox: torch.Tensor,
        enlargement_factor: torch.Tensor,
        translation_factor: torch.Tensor,
        beyond_border_shift: float,
    ):
        """
        ...
        """
        assert (
            face_bbox.shape[:-1] == enlargement_factor.shape
        ), f"Bad bounding box shape {face_bbox.shape}"
        ...
        wiggle_room_x = 0.5 * torch.abs(size - bbox_w) + beyond_border_shift * torch.minimum(
            size, bbox_w
        )
        wiggle_room_y = 0.5 * torch.abs(size - bbox_h) + beyond_border_shift * torch.minimum(
            size, bbox_h
        )
        tx = wiggle_room_x * rx
        ty = wiggle_room_y * ry
    ...
    def _compute_point_transform_from_roi(
        self, B: Tuple[int], new_roi: torch.Tensor, new_size: int
    ):
        return Affine2d.range_remap_2d(
            ...
            outmax=torch.full(
                B + (2,), fill_value=new_size, device=new_roi.device, dtype=torch.float32
            ),
        )
    ...
    def _center_rotation_tr(self, rotations: Tensor):
        ...
        tr_denorm = position_unnormalization(self.new_size, self.new_size).to(
    ...
    def __call__(self, sample: Batch):
        ...
        view_roi = self._compute_view_roi(
            roi, params.scales, params.translations, self._max_beyond_border_shift
        )
        ...
        if params.angles.item() != 0.0:
            image_transform_function = lambda img: affine_transform_image_cv2(
                img, tr, self.new_size, downfilter=params.downfilter, upfilter=params.upfilter
            )
        else:
            image_transform_function = lambda img: croprescale_image_cv2(
                img, view_roi, self.new_size, downfilter=params.downfilter, upfilter=params.upfilter
            )
...
def horizontal_flip_and_rot_90(p_rot: float, sample: Batch):
    ...
    rot_dir = np.random.choice([-1, 0, 1], p=[p_rot / 2.0, (1.0 - p_rot), p_rot / 2.0])
    ...
    if rot_dir != 0:
        tr = (
            tr
            @ Affine2d.range_remap_2d([-1.0, -1.0], [1.0, 1.0], [0.0, 0.0], [w, h])
            @ Affine2d.trs(angles=torch.tensor(rot_dir * np.pi * 0.5, dtype=torch.float32))
            @ Affine2d.range_remap_2d([0.0, 0.0], [w, h], [-1.0, -1.0], [1.0, 1.0])
        )
    if do_flip:
        tr = tr @ Affine2d.range_remap_2d([0.0, 0.0], [w, h], [w, 0], [0, h])
