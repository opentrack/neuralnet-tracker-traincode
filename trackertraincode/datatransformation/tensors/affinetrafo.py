...
def position_normalization(w: int, h: int):
    return Affine2d.range_remap_2d([0.0, 0.0], [w, h], [-1.0, -1.0], [1.0, 1.0])


def position_unnormalization(w: int, h: int):
    return Affine2d.range_remap_2d([-1.0, -1.0], [1.0, 1.0], [0.0, 0.0], [w, h])


def handle_backtransform_insertion(
    sample: dict, W: int, H: int, tr: Affine2d, type: str = "tensor"
):
...
def transform_points(tr: Affine2d, points: torch.Tensor):
    ...
    new_shape = (
        batch_dimensions
        + tuple(1 for _ in range(len(points.shape) - len(batch_dimensions) - 1))
        + (2, 3)
    )
    m = m.view(*new_shape)
    if points.size(-1) == 2:
...
def transform_keypoints(tr: Affine2d, points: torch.Tensor):
    ...
    det = tr.det
    if points.shape[-1] == 3 and torch.any(det < 0.0):
        mask = det < 0.0
        flipped = out[mask, ...][:, flip_map, :].clone(memory_format=torch.contiguous_format)
        out[det < 0.0] = flipped
    elif points.shape[-1] == 2 and det < 0.0:
