...
from trackertraincode.datatransformation.tensors.image_geometric_cv2 import _extract_size_tuple
...
def _fixup_image_format_for_resample(img: Tensor):
    ...
    def restore(x: Tensor):
        if original_dtype == torch.uint8:
            x = x.clip(0.0, 255.0).to(dtype=torch.uint8)
        return x
...
def croprescale_image_torch(img: Tensor, roi: Tensor, new_size: Union[int, Tuple[int, int]]):
    ...
    assert img.shape[-2:] == (
        new_h,
        new_w,
    ), f"Torchvision resize failed. Expected shape ({new_h,new_w}). Got {img.shape[-2:]}."
...
def affine_transform_image_torch(
    tmp: Tensor, tr: Affine2d, new_size: Union[int, Tuple[int, int]], antialias=False
):
    ...
    if antialias:
        ...
        # This gets inefficient as the filter size gets larger
        scaling = tr.scales
        sampling_distance = 1.0 / scaling
        ks = 0.4 * sampling_distance
        intks = max(3, int(ks * 3))
        intks = intks if (intks & 1) == 1 else (intks + 1)
        tmp = kornia.filters.gaussian_blur2d(
            tmp, (intks, intks), (ks, ks), border_type="constant", separable=True
        )
    grid = F.affine_grid(
        tr_normalized.to(device=tmp.device), [1, C, new_h, new_w], align_corners=False
    )
    if 0:  # Debugging
        from matplotlib import pyplot

        pyplot.imshow(tmp[0, 0], extent=[-1.0, 1.0, 1.0, -1.0])
