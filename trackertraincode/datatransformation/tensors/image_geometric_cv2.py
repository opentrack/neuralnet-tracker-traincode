...
def _resize(img, new_w, new_h, downfilter: DownFilters, upfilter: UpFilters) -> NDArray[Any]:
    ...
    filter = downfilter if scale_factor < 1.0 else upfilter
    if filter not in ("gaussian", "hamming"):
        ...
        return cv2.resize(
            _apply_antialias_filter(img, scale_factor, filter),
            dsize=(new_w, new_h),
            interpolation=cv2.INTER_LINEAR,
        )


def affine_transform_image_cv2(
    img: Tensor,
    tr: Affine2d,
