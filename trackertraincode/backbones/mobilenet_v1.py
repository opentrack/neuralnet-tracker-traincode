...
class DepthWiseBlock(nn.Module):
    def __init__(
        self, inplanes, planes, stride=1, momentum=0.1, stochastic_depth=None, use_blurpool=True
    ):
...
class MobileNet(nn.Module):
    def __init__(
        ...
        def block(inplanes, planes, stride=1):
            return DepthWiseBlock(
                inplanes * widen_factor,
                planes * widen_factor,
                stride=stride,
                momentum=momentum,
        ...
        self.conv1 = nn.Conv2d(
            input_channel, int(32 * widen_factor), kernel_size=5, stride=2, padding=2, bias=False
        )
        ...
        self.num_intermediate_features = [int(c * widen_factor) for c in [64, 128, 256, 512, 1024]]
...
class UpsampleBlock(nn.Sequential):
    def __init__(self, inplanes, planes, momentum=0.1):
        ...
        conv_dw = nn.ConvTranspose2d(
            inplanes, inplanes, kernel_size=3, padding=1, stride=2, groups=inplanes, bias=False
        )
        ...
        super(UpsampleBlock, self).__init__(conv_dw, bn_dw, relu, conv_sep, bn_sep, relu)
...
class InvMobileNet(nn.Sequential):
    def __init__(self, num_classes, widen_factor=1, momentum=0.1):
        ...
        head = nn.ConvTranspose2d(
            int(32 * widen_factor), num_classes, kernel_size=3, stride=2, padding=1, bias=True
        )
