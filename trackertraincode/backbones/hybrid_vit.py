...
class HybridVitBackbone(nn.Module):
    def __init__(self, num_heads):
        ...
        self.position = nn.Parameter(torch.randn((1,self.position_enc_dim, self.featuremap_size, self.featuremap_size), requires_grad=True))
        self.queries = nn.Parameter(torch.randn((1,self.num_queries,self.transformer_dim), requires_grad=True))
    ...
    def _add_position_encoding(self, z):
        ...
        # ], dim=1)
        return torch.cat([
            z, self.position.expand(B,-1,-1,-1)
        ], dim=1)
