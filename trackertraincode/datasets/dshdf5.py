...
class ImageVariableLengthBufferDs(ImageDs):
    ...
    def __setitem__(self, index: int, value):
        assert (isinstance(value, np.ndarray) and value.dtype == np.uint8) or isinstance(
            value, PIL.Image.Image
        )
        ...
        if len(value.shape) in (2, 3):
            ...
            if which_image_format(value) != self._format:
                raise ValueError(
                    f"Buffer for lossy/lossless data must be encoded as jpg/png, got {which_image_format(value)}"
                )
    ...
    @staticmethod
    def create(
        g: h5py.Group, name: str, size: int, maxsize: Optional[int] = None, lossy: bool = True
    ):
        ds = g.create_dataset(
            name,
            (size,),
            variable_length_hdf5_buffer_dtype,
            maxshape=(maxsize,),
            chunks=_chunk_shape((size,), (maxsize,)),
        )
...
class ImagePathDs(ImageDs):
    ...
    def _find_filenames(ds: h5py.Dataset):
        ...
        extensions_to_try = (
            ...
            if (splitext(first.lower())[1] not in supported_extensions)
            else ("",)
        )
...
def create_dataset(
    g: h5py.Group, name: str, shape: Tuple = None, dtype=None, maxshape: Tuple = None, data=None
):
    if data is not None:
    ...
    return g.create_dataset(
        name, shape, dtype, chunks=_chunk_shape(shape, maxshape), maxshape=maxshape, data=data
    )
...
def _quantize(values: np.ndarray):
    ...
    buffer = ((values - minval) / (maxval - minval + 1.0) * 256).astype(np.uint8)
    return np.squeeze(minval), np.squeeze(maxval), buffer


def _dequantize(minval, maxval, buffer, shape):
    buffer = buffer / 256.0 * (maxval - minval + 1) + minval
...
class QuantizedVarsizeArrayDs(object):
    ...
    def create(
        ...
        name: str,
        size: int,
        sample_dimensionality: int,
        maxsize: Optional[int] = None,
    ):
        dt = np.dtype(
            [
                ("shape", "i4", (sample_dimensionality,)),
                ("minval", "f4"),
                ("maxval", "f4"),
                ("buffer", variable_length_hdf5_buffer_dtype),
            ]
        )
        ds = g.create_dataset(
            name, (size,), chunks=_chunk_shape((size,), (maxsize,)), maxshape=(maxsize,), dtype=dt
        )
...
def open_dataset(g: h5py.Group, name: str) -> MaybeWrappedH5Dataset:
    ...
    if typeattr == DatasetEncoding.varsize_image_buffer:
        return ImageVariableLengthBufferDs(ds)
    raise RuntimeError(
        f"Cannot create dataset wrapper. Unknown value of attribute 'storage': {typeattr}"
    )


def open_all_datasets(
    root: h5py.Group, whitelist: Whitelist
) -> List[Tuple[str, MaybeWrappedH5Dataset]]:
    opened = []
    for ds in glob_hdf_datasets(root, whitelist):
        opened.append((basename(ds.name), open_dataset(root, ds.name)))
    assert len(set(k for k, _ in opened)) == len(
        opened
    ), "Found datasets must have unique base names."
    return opened
...
class TorchHdf5DatasetBase(Dataset):
    ...
    def _init_from_file(
        self, f: h5py.File, whitelist: Whitelist
    ) -> List[Tuple[str, MaybeWrappedH5Dataset]]:
        names_datasets = open_all_datasets(f, whitelist)
        lengths = [len(v) for _, v in names_datasets]
        assert lengths and all(
            l == lengths[0] for l in lengths
        ), f"Inconsistent lengths among data: {[k for k,v in names_datasets]}"
    ...
    def __getitem__(self, index):
        ...
        return [
            (name, self._get_field(dataset, index))
            for name, dataset in self._names_datasets.items()
        ]
