...
class SobolChoices(object):
    def __init__(self, weights, seed=None):
        ...
        assert torch.abs(self.accum[-1] - 1.0) < 1.0e-6
...
class ConcatDatasetSampler(Sampler[T_co]):
    ...
    def _generate_item(
        self, sampler_output: Union[List[int], int], dataset_start_index: int
    ) -> T_co:
    ...
    def __iter__(self) -> Iterator[T_co]:
        ...
        iters = [utils.cycle(ds) for ds in self.samplers]
...
def make_concat_dataset_item_sampler(
    ...
    return ConcatDatasetSampler(dataset, wrapped, PseudoRandomChoices(weights), stop_after)
