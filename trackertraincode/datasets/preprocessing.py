...
def which_image_format(buffer: np.ndarray) -> ImageFormat:
    ...
    return {"jpeg": ImageFormat.JPG, "png": ImageFormat.PNG}[kind]
...
def imencode(img: NDArray[np.uint8], format=ImageFormat.JPG, quality=None) -> np.ndarray:
    ...
    """
    cv_format = {ImageFormat.JPG: ".JPEG", ImageFormat.PNG: ".PNG"}[format]
...
def imrescale(img: Union[NDArray[np.uint8], Image.Image], factor: float):
    ...
    if isinstance(img, np.ndarray):
        out: NDArray[np.uint8] = cv2.resize(
            img,
            (new_w, new_h),
            interpolation=cv2.INTER_AREA if factor < 1.0 else cv2.INTER_BILINEAR,
        )
        return out
    elif isinstance(img, Image.Image):
        return img.resize((new_w, new_h), resample=Image.HAMMING, reducing_gap=3.0)
...
def imshape(img: Union[NDArray[np.uint8], Image.Image]) -> Tuple[int, int]:
    ...
    return (
        tuple(map(int, img.shape[:2])) if isinstance(img, np.ndarray) else (img.height, img.width)
    )
...
def compute_padding(roi, w, h):
    ...
    return max(max(-x0, 0), max(-y0, 0), max(x1 - w, 0), max(y1 - h, 0))
...
def sanity_check_landmarks(coord, rotation, pt3d_68, params=None, reltol=0.4, img=None):
    if params is None:
        ...
        f_shp, f_exp = params
    expected = compute_keypoints(f_shp, f_exp, coord[2], rotation, coord[0], coord[1])
    if not np.allclose(expected, pt3d_68, rtol=0.0, atol=coord[2] * reltol):
        print(
            "Large deviation between base shape and point labels detected. Check for coordinate flips. Click the window away if the sample is fine."
        )
        ...
        ax.scatter(*pt3d_68, cmap="rainbow", c=np.linspace(0.0, 1.0, 68), label="labels")
        ax.scatter(
            *expected, cmap="rainbow", c=np.linspace(0.0, 1.0, 68), marker="x", label="model"
        )
        ax.legend()
        if img is not None:
            ax = fig.add_axes([0.0, 0.0, 0.33, 0.33])
            ax.imshow(img)
        pyplot.show()
