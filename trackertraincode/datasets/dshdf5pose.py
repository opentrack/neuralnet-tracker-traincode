...
from trackertraincode.datasets.dshdf5 import (
    TorchHdf5DatasetBase,
    Whitelist,
    MaybeWrappedH5Dataset,
    ImageVariableLengthBufferDs,
    create_dataset,
)
...
def _transfrom_to_pose_sample(
    sample: List[Tuple[str, torch.Tensor]], dataclass: Tag, categories_mapping: Field2Categories
):
...
class Hdf5PoseDataset(TorchHdf5DatasetBase):
    def __init__(
        ...
        self.coord_convention_id = coord_convention_id
...
class Hdf5PoseVideoDataset(TorchHdf5DatasetBase):
    def __init__(
        ...
        whitelist = whitelist or default_whitelist
        super().__init__(filename, monochrome=monochrome, whitelist=whitelist)
        self.dataclass = dataclass
        self.transform = (lambda x: x) if transform is None else transform
