...
class Metadata:
    ...
    def image_wh(self):
        return (
            self._imagesize
            if isinstance(self._imagesize, tuple)
            else (self._imagesize, self._imagesize)
        )
...
class Batch:
    ...
    def with_batchdim(self) -> "Batch":
        ...
        if self.meta.seq is not None:
            ...
            return Batch(meta, ((k, v[None, ...]) for k, v in self.items()))
    ...
    def iter_sequences(self) -> Iterator["Batch"]:
        ...
        for a, b in self.meta.sequence_start_end:
            ...
            meta.batchsize = 0
            meta.seq = (0, b - a)
            yield Batch(meta, ((k, v[a:b, ...]) for k, v in self.items()))
    ...
    def to(self, *args, **kwargs):
        assert all(
            isinstance(x, torch.Tensor) for x in self._data.values()
        ), "Only applicable to PyTorch"
    ...
    class Collation(object):
        ...
        def _collate_single_class(self, samples: List["Batch"]) -> "Batch":
            ...
            return (
                self._collate_stills(samples, first)
        ...
        def _combine_metadata(self, samples: List["Batch"], first: "Batch") -> Metadata:
            ...
            if first.meta.seq is None:
                ...
                seq = np.concatenate(
                    ...
                    + [np.asarray(s.meta.seq[1:]) + o for s, o in zip(samples, offsets)]
                ).tolist()
                # Adjust metadata
                meta = copy.copy(first.meta)
        ...
        def _combine_samples(
            self, samples: List["Batch"], first: "Batch"
