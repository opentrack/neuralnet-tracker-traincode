...
def draw_roi(img, roi, color, linewidth):
    cv2.rectangle(
        img, (round(roi[0]), round(roi[1])), (round(roi[2]), round(roi[3])), color, linewidth
    )
...
def draw_semseg_logits(semseg: np.ndarray):
    ...
    H, W, C = probs.shape
    colored = np.sum(
        _ibug_semseg_colors[None, None, :, :].astype(np.float32) * probs[..., None], axis=-2
    )
...
def _draw_sample(
    ...
    if "pt3d_68" in sample:
        draw_points3d(img, sample["pt3d_68"], linewidth - 1, color, labels)
    if "pt2d_68" in sample:
        draw_points3d(img, sample["pt2d_68"], linewidth - 1, color, labels)
