...
def tomatrix(sixdrot : Tensor):
    ...
    badness = torch.linalg.norm((torch.matmul(out, out.transpose(-2, -1)) - eye).flatten(start_dim=-2), ord=torch.inf, dim=-1)
    #out[badness > 1.e-3,:,:] = eye
    out = torch.where(badness[:,None,None] > 1.e-3, eye, out)
