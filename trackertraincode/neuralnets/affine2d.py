...
class Affine2d(object):
    ...
    @staticmethod
    def trs(
        translations: MaybeTensor = None, angles: MaybeTensor = None, scales: MaybeTensor = None
    ):
    ...
    @staticmethod
    def range_remap(
        inmin: torch.Tensor, inmax: torch.Tensor, outmin: torch.Tensor, outmax: torch.Tensor
    ):
...
def roi_normalizing_transform(roi: torch.Tensor):
    ...
    roi = roi.view(-1, 4)
    N, _ = roi.shape
    out_min = torch.tensor([-1.0, -1.0], dtype=roi.dtype, device=roi.device).expand(N, 2)
    out_max = torch.tensor([1.0, 1.0], dtype=roi.dtype, device=roi.device).expand(N, 2)
