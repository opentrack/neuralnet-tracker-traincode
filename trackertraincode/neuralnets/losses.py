...
def smooth_geodesic_distance(pred, target):
    smooth_zone = 1.0 * torch.pi / 180.0  # One degree
    normed_delta = torchquaternion.geodesicdistance(pred, target)
    return (
        F.smooth_l1_loss(
            normed_delta, torch.zeros_like(normed_delta), reduction="none", beta=smooth_zone
        )
        / torch.pi
    )
...
class QuaternionNormalizationSoftConstraint(object):
    ...
    def __call__(self, pred, sample):
        ...
        norm_loss = torch.square(1.0 - norm_loss)
        return norm_loss
...
class Points3dLoss(nn.Module):
    ...
    def _eval_on_points(self, pred: torch.Tensor, target: torch.Tensor):
        ...
        pointwise = self.loss_obj(
            pred[..., : self.pointdimension], target[..., : self.pointdimension]
        ).sum(dim=-1)
